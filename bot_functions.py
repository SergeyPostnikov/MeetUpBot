...
def get_faq_question(message: telebot.types.Message, call):
    user = payload[message.chat.id]
    bot.edit_message_text(chat_id=message.chat.id, message_id=user['msg_id_2'],
                          text='Чтобы задать вопрос по докладу необходимо в главном меню выбрать пункт "задать вопрос докладчику".\n'
                                'Я покажу вам какой доклад идет в данный момент.\n'
                                'Чтобы отправить вопрос текущему спикеру - нажмите "задать вопрос".\n'
                                'Если интересует другой доклад нажмите - "Найти другого спикера", я выведу все доклады мероприятия и вы сможете выберать интересующий.\n'
                                'Для возврата в главное меню жми "вернуться в меню".',
...
def get_faq_communicate(message: telebot.types.Message, call):
    user = payload[message.chat.id]
    bot.edit_message_text(chat_id=message.chat.id, message_id=user['msg_id_2'],
                          text='Для общения с другими участниками необходимо в главном меню выбрать пункт "общаться с другими", если вы еще не заполняли свою анкету я попрошу вас это сделать.\n'
                          'В следующем меню выйдут анкеты участников с именами и профессиями.\n'
                          'Нажмите на заинтересовавшую вас анкету чтобы посмотреть подробную информацию человека, если заинтересовались перейдите к личному общению через пункт "Написать в личку" или вернитесь назад к списку участников.\n'
                          'Если в списке участников никто не заинтересовал - нажмите на пункт "еще анкеты" и я покажу следующих участников.\n'
                          'Для возврата в главное меню жми "вернуться в меню".',
                          reply_markup=markup_main_menu)
...
def get_faq_start_report(message: telebot.types.Message, call):
    ...
    bot.edit_message_text(chat_id=message.chat.id, message_id=user['msg_id_2'],
                          ...
                          'Во время доклада вы сможете получить список вопросов по вашему докладу, нажав на кнопку "Вопросы по докладу".\n'
                          'После ответа на вопрос необходимо пометить его отвеченным, нажав "Закрыть вопрос".\n'
                          'Для получения следующего нажмите на кнопку "Следующий вопрос".\n'
                          'Для возврата в главное меню жми "вернуться в меню".\n'
                          'Для завершения доклада необходимо нажать "начать/закончить доклад" и "Завершить доклад" в следующем меню.\n'
                          'Если вы не являетесь спикером на текущем митапе, то будете возвращены в главное меню.',
                          reply_markup=markup_main_menu)
