...
def get_faq_start_report(message: telebot.types.Message, call):
    user = payload[message.chat.id]
    bot.edit_message_text(chat_id=message.chat.id, message_id=user['msg_id_2'],
                          text='Для начала доклада нажмите в главном меню кнопку "начать/закончить доклад", я сообщу вам через сколько начнется ваш доклад. Во время доклада вы сможете получить список вопросов по вашему докладу, нажав на кнопку "Прочитать вопросы". После ответа на вопрос можно получить следующий нажатием на кнопку "Следующий вопрос". Для возврата в главное меню жми "вернуться в меню". Если во время доклада выйти в главное меню и снова нажать "начать/закончить доклад" чтобы завершить свой доклад. Если вы не являетесь спикером на текущем митапе, то будете возвращены в главное меню.',
                          reply_markup=markup_main_menu)
